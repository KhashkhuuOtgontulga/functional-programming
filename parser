...
localVars = token $ (do local <- many (do typename
                                          var <- identifier
                                          vars <- many (do symbol ","
                                                           identifier)
                                          symbol ";"
...
funcDef = do typename            
             ...
             char '{'            
             local <- localVars 
             r <- returnParser <|> return ""
             char '}'
             return (name, params, local, r)
...
intParser = State $ \(c:cs) -> Just (ord c - 48, cs)

assignment :: Parser ()
assignment = token $ (do identifier
                         char '='
                         integer
                         char ';'
                         return ())
